...
class VocabSRSReview {
  ...
  async loadReviewData() {
    try {
      ...
      // First check if VocabUtils is available
      if (!window.VocabUtils || !window.VocabUtils.VocabStorage) {
        throw new Error('VocabUtils is not available. Please check if utils.js is loaded.');
      }
      
      // Get all words first to see what we have
      const allWords = await window.VocabUtils.VocabStorage.getAllWords();
      console.log(`📚 Total words in vocabulary: ${allWords.length}`);
      
      if (allWords.length === 0) {
        this.showError('No words in your vocabulary. Please add some words first.');
        return;
      }
      
      // Log sample words for debugging
      if (allWords.length > 0) {
        console.log('📋 Sample words:', allWords.slice(0, 3).map(word => ({
          word: word.word,
          srs: word.srs,
      ...
      console.log(`⏰ Due words found: ${this.currentReviewWords.length}`);
      ...
      if (this.currentReviewWords.length === 0) {
        ...
        const nextDueWord = allWords
          .filter(word => word.srs && word.srs.nextReview)
          .sort((a, b) => {
            const aTime = typeof a.srs.nextReview === 'string' ? 
              new Date(a.srs.nextReview).getTime() : a.srs.nextReview;
            const bTime = typeof b.srs.nextReview === 'string' ? 
              new Date(b.srs.nextReview).getTime() : b.srs.nextReview;
            return aTime - bTime;
          })[0];
        
        if (nextDueWord) {
          const nextTime = typeof nextDueWord.srs.nextReview === 'string' ? 
            new Date(nextDueWord.srs.nextReview) : new Date(nextDueWord.srs.nextReview);
          const timeUntilNext = window.VocabUtils.TimeUtils.formatTimeUntilReview(nextTime.toISOString());
          
          this.showError(`No words are due for review right now. Next word "${nextDueWord.word}" is due in ${timeUntilNext}.`);
        } else {
          this.showError('No words are scheduled for review. All your words may be new - they should be available for review immediately.');
        }
        return;
      }
      
      console.log('✅ Review data loaded successfully');
      console.log('🎯 Words for review:', this.currentReviewWords.map(w => w.word));
  ...
  async completeReview() {
    ...
    if (window.VocabAnalytics) {
      try {
        ...
        await window.VocabAnalytics.endSession(enhancedStats);
      } catch (error) {
