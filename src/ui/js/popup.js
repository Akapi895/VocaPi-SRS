...
class VocabSRSPopup {
  ...
  async initializeAnalytics() {
    try {
      if (typeof VocabAnalytics !== 'undefined') {
        ...
        console.log('✅ Analytics initialized in popup');
      } else {
        console.warn('⚠️ VocabAnalytics not available');
      }
    } catch (error) {
    ...
    try {
      ...
      if (typeof FastGamificationWidget !== 'undefined') {
        ...
        try {
          ...
          return;
        } catch (fastWidgetError) {
          console.warn('⚠️ Fast widget failed, trying fallback:', fastWidgetError);
        }
      }
      ...
      if (!this.gamificationUI && typeof GamificationUI !== 'undefined') {
        ...
        try {
          ...
          console.warn('⚠️ GamificationUI init failed:', initError);
          this.gamificationUI = null;
        }
      }
      ...
      if (this.gamificationUI) {
        try {
          ...
          console.error('❌ Failed to render gamification widget:', renderError);
          this.showGamificationError(container, 'Rendering failed');
        }
      } else {
      ...
      this.showGamificationError(container, error.message || 'Unknown error');
    }
  }

  showGamificationError(container, errorMessage) {
    // Show minimal error state with retry option
    container.innerHTML = `
      <div style="
        ...
        font-size: 12px;
      ">
        <div style="margin-bottom: 8px;">🎮 Gamification Error</div>
        <div style="opacity: 0.8; margin-bottom: 12px;">${errorMessage}</div>
        <button 
          onclick="this.parentElement.parentElement.innerHTML='<div style=&quot;padding: 16px; text-align: center; color: rgba(255,255,255,0.7);&quot;>🔄 Retrying...</div>'; setTimeout(() => window.location.reload(), 1000);"
          style="
    ...
    try {
      ...
      if (this.analytics) {
        try {
          ...
          console.warn('⚠️ Error loading analytics data:', analyticsError);
        }
      }
  ...
  updateAnalyticsDisplay(data) {
    try {
      // Update level and XP if elements exist
      const levelElement = document.querySelector('.gamification-level');
      if (levelElement) {
        levelElement.textContent = `Level ${data.level || 1}`;
      }
      
      const xpElement = document.querySelector('.gamification-xp');
      if (xpElement) {
        xpElement.textContent = `${data.totalXP || 0} XP`;
      }
      
      const streakElement = document.querySelector('.study-streak');
      if (streakElement) {
        streakElement.textContent = `${data.currentStreak || 0} days`;
      }
      
      const accuracyElement = document.querySelector('.accuracy-rate');
      if (accuracyElement) {
        accuracyElement.textContent = `${data.accuracyRate || 0}%`;
      }
      
      console.log('✨ Analytics display updated successfully');
    } catch (error) {
      console.warn('⚠️ Error updating analytics display:', error);
    }
  }
  ...
  async openSettings() {
    try {
      ...
      chrome.tabs.create({
        url: chrome.runtime.getURL('src/ui/html/options.html')
      });
